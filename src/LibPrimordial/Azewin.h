...
/*
    Azewin is a modular, mean and lean replacement for <Windows.h>.
    Including that file is a massive headache!
    It contains countless macros that nobody really wants. It uses so many names it's unreal.
    Also, it's HUGE! Somewhere north of 50.000 lines of code. That is not free. 
    It has to be parsed and compiled. Most of that is absolutely useless and just waste of time.
    Also I hate the typedefs that MS uses, so this also get's rid of them, 
    and replaces them with modern type names. 
    As an additional benefit I've added a lot of inline documentation and external links.
    
    Long story short; Don't include <Windows.h>!
    
    In addition to providing most of what anyone needs from Windows.h, 
    Azewin contains the meat from some of the other MS header files that, I often use.
...
#define AZEWIN_COMPILE_PSAPI    0
#define AZEWIN_COMPILE_DSOUND   0
...
typedef s64 (__stdcall *WWindowProcedure)(WWindowHandle window, u32 message, u64 wparam, s64 lparam); 
...
struct WProcess_Memory_Counters_EX
{
    u32 size;
    u32 page_fault_count;
    u64 peak_working_set_size;
    u64 working_set_size;
    u64 quata_peek_paged_pool_usage;
    u64 quate_paged_pool_usage;
    u64 quota_peak_non_paged_pool_usage;
    u64 quate_non_paged_pool_usage;
    u64 pagefile_usage;
...
*/
extern "C" __declspec(dllimport) u32 __stdcall GetCurrentProcessId(void);
#endif
...
*/
extern "C" __declspec(dllimport) b32 __stdcall WriteFile(WHandle file, void* buffer, u32 number_of_bytes_to_write, u32* number_of_bytes_written, void*);
#endif
...
*/
extern "C" __declspec(dllimport) WHandle __stdcall OpenProcess(u32 desired_access, b32 inherit_handle, u32 process_id);
#endif



// --- GetProcessMemoryInfo import
// Source: psapi
#if AZEWIN_COMPILE_PSAPI || AZEWIN_DEFAULT_COMPILE || 0
/*
...
*/
extern "C" __declspec(dllimport) b32 __stdcall GetProcessMemoryInfo(WHandle process, WProcess_Memory_Counters_EX* mem_counters, u32 mem_counters_size);
#endif
...
#if AZEWIN_COMPILE_DSOUND || AZEWIN_DEFAULT_COMPILE || 0
...
constexpr u32 CW_USEDEFAULT  = ((u32)0x80000000);
...
constexpr u32 PFD_UNDERLAY_PLANE   = (u32)((s32)(-1));
...
enum WMem
{
    ...
    // Mark the segment of the virtual address space as reserved.
    // Meaning that nobody else can can get that part of the address space.
...
constexpr u32 INVALID_FILE_SIZE         = u32(0xFFFFFFFF);
